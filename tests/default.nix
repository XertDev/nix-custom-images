...
let
  ...
  subtypeNames = builtins.listToAttrs (map (val: {
    name = val;
    value = builtins.attrNames images."${val}";
  }) imageNames);

  subtypesCount = let
    perSubtypeCount = builtins.mapAttrs (_: v: builtins.length v) subtypeNames;
  in lib.attrsets.foldlAttrs (acc: _: v: acc + v) 0 perSubtypeCount;

  subtypeFunctions = lib.lists.flatten (map (val:
    (lib.attrsets.mapAttrsToList (k: _: {
      path = [ val k ];
      args = { };
    }) images.${val})) imageNames);
in pkgs.runCommand "image-build-check" { } ''
  ...
  ${lib.strings.concatStringsSep "\n" (map (val: ''
    echo "${lib.strings.concatStringsSep "." val.path} -> ${
      (lib.attrsets.getAttrFromPath val.path images) val.args
    }"
  '') subtypeFunctions)}
